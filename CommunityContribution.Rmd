...
When we wish to concentrate on a single entity or individual's data, especially when it involves multiple dimensions and attributes, a spider/radar graph proves to be a valuable choice. This type of graph is particularly useful for performance evaluation or assessment as it aids in evaluating the strengths and weaknesses of individuals or entities by visually representing their performance in various areas.
...
When our focus is on a specific data type, that is words, a wordcloud can be particularly advantageous for content analysis. It enables the swift identification of keywords or themes.

https://r-graph-gallery.com/196-the-wordcloud2-library.html
https://cran.r-project.org/web/packages/ggwordcloud/vignettes/ggwordcloud.html

```{r Wordcloud}
# Install wordcloud2 package
library(wordcloud2)
...
```{r Customize wordcloud}
# Chaneg size, shape
wordcloud2(demoFreq, size = 0.6, shape = 'star')
```

```{r Wordcloud using ggplot}
library(ggwordcloud)
ggplot(demoFreq[1:100,], aes(label = word, size = freq)) +
  geom_text_wordcloud() +
  theme_minimal()
```

```{r Customize wordcloud using ggplot}
ggplot(demoFreq[1:100,],aes(label = word, 
                            size = freq,
                            color = freq
                            )
       ) +
